...
//! 実行時の更新確認は共有フォルダ 更新置き場\KENBAN\ だけを見る。信頼基盤は minisign 署名。
//! 更新先・公開鍵はアドレス直書き（割符外付けは終盤フェーズで導入予定）。最終ゲートは minisign 検証。

use serde::Serialize;
use std::path::PathBuf;

const APP_UPDATE_SUBDIR: &str = "KENBAN";
const UPDATE_LOCAL_DIR: &str =
    r"G:\共有ドライブ\CLLENN\編集部フォルダ\編集企画部\編集企画_C班(AT業務推進)\DTP制作部\App_installer";
/// KENBAN updater 公開鍵（minisign・二重base64）。秘密鍵は鍵フォルダのみ。
const UPDATER_PUBKEY: &str = "dW50cnVzdGVkIGNvbW1lbnQ6IG1pbmlzaWduIHB1YmxpYyBrZXk6IDdCNDMwOEQ0NkQxMjI1REMKUldUY0pSSnQxQWhEZTNSNUEyVGRxQlVEQXdpZjN4Ri8xemlwMktHdzFyTDl5VmV0aklQOTVDK2cK";

...
## v2.3.2 変更点（2026-06-10）
並列ビューアーのバグ修正 + ズーム機能追加。

### バグ修正
- **同期ボタンで同期に戻れない**: 非同期モードで「同期」ボタンを押してもポップアップが開くだけで同期へ戻らなかった問題を修正。「同期」ボタンクリックで即座に同期モードへ復帰（現在のページ位置を維持）するようにした。「ページを合わせて再同期」等のオプションは非同期時に表示される小さな ▾ キャレットボタンから利用可能（[src/components/ParallelViewer.tsx](src/components/ParallelViewer.tsx) の同期/非同期トグル）。
- **非同期→同期に戻したときの状態残留**: 非同期で片パネルだけ拡大/移動した状態のまま同期に戻ると片側だけ拡大されたままになっていたため、同期モードへ入った瞬間に両パネルのズーム/パンを等倍・原点へリセットする `useEffect` を追加（[src/App.tsx](src/App.tsx)）。
- **指示エディタで保存できない**: 指示エディタ（ScreenshotEditor）に渡される画像が `convertFileSrc`（asset プロトコル URL）由来のため、`crossOrigin` 指定なしで canvas へ描画すると canvas が tainted となり、保存/コピー時の `toBlob()` が SecurityError で失敗していた。画像ロード時に `img.crossOrigin='anonymous'` を付与して解決（CORS 読み込み失敗時は crossOrigin なしで再読込し表示は維持するフォールバック付き）。[src/components/ScreenshotEditor.tsx](src/components/ScreenshotEditor.tsx)

### 追加機能
- **並列ビューアーの画像ズームイン/アウト**: 以下の操作に対応（同期モードは両パネル連動、非同期モードは対象パネルのみ）。
  - **Ctrl + マウスホイール**でズームイン/アウト（A/B どちらのパネル上でも）
  - ツールバーに **− / 倍率% / ＋** のズームコントロールを追加（% クリックで等倍リセット）
  - **Ctrl + / − / 0** を同期モードでも有効化（従来は非同期モードのみ）
  - 同期モードでもズーム倍率オーバーレイ表示、ドラッグでのパンも同期モードでは両パネル連動
  - ヘルプ（?）にショートカット説明を追記

### バージョン
- 2.3.1 → 2.3.2（package.json / Cargo.toml / tauri.conf.json）
